...
def cross_validate_feature_selection(
    ...
    """
    Perform k-fold cross-validation for feature selection.
    Optionally supports sampling and repartitioning for memory control.
    """
    print('Starting cross validation feature selecction...')
    # Input validation
    ...
    if verbose:
        print(f"- Total parameter & fold combinations: {len(param_grid_built) * num_folds}")
        print("Running cross-validation with hyperparameter tuning...")
    ...
    print("Cross validation feature selection completed ")
    data.unpersist()
    # return best_model, results_df, cv_model
    return cv_model
